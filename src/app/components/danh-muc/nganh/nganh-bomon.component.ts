...
export class NganhBomonComponent implements OnInit , OnDestroy {
    ...
    loadData ( paged : number = 1 , resetPaginator : boolean = true ) : void {
        ...
        this.danhMucService.loadNganhBomon( 'nganh' , this._search , paged , this.dataTable.paginator.rows() , donviId ).pipe(
            takeUntil( this.destroy$ )
        ).subscribe( {
            ...
            error : () : void => {
                ...
                this.notification.toastError( 'Không tải được danh mục ngành' );
            }
        } );
    }
    ...
    deleteRow ( row : NganhBomon ) : void {
        this.notification.confirmDelete( 1 ).subscribe( ( confirmed : boolean ) : void => {
            ...
            this.danhMucService.deleteNganhBomon( row.id ).subscribe( {
                next  : () : void => {
                    this.notification.toastSuccess( 'Xóa ngành thành công' );
                    this.loadData( this._temp.paged , false );
                } ,
                error : () : void => {
                    this.notification.toastError( 'Xóa ngành thất bại' );
                }
            } );
        } );
    }
    ...
    deleteSelectedRows () : void {
        ...
        this.notification.confirmDelete( selected.length ).subscribe( ( confirmed : boolean ) : void => {
            ...
            forkJoin( requests ).subscribe( {
                next  : () : void => {
                    this.notification.toastSuccess( `Xóa thành công ${ selected.length } ngành` );
                    this.loadData( 1 , true );
                } ,
                error : () : void => {
                    this.notification.toastError( 'Xóa danh sách ngành thất bại' );
                }
            } );
        } );
    }
