...
export class IctuFileService {
	...
	uploadWithProgress( file : File , fileAttributes? : FileUploadAttributes , otherLink? : string ) : Observable<UploadInfo> {
		return this.http.post<DtoObject<ICTUStandardFile[]>>( otherLink ?? this.fileHostingServiceApi , fileToFormData( file , fileAttributes ) , {
			...
			distinctUntilChanged( ( previous : UploadInfo , current : UploadInfo ) : boolean => previous?.progress === current.progress && previous?.state === current.state )
	...
	public createFolder( info : Partial<IctuFile> ) : Observable<number> {
	...
	public updateFileInfo( fileName : string , info : Partial<IctuFile> ) : Observable<number> {
		return this.http.put<DtoObject<number>>( ''.concat( this.awsApi , fileName ) , info ).pipe(
			map( ( response : DtoObject<number> ) : number => response.data )
		);
	}
	
	public loadLibraryChildren( parent_id : number , queryParams : IctuQueryParams = {} ) : Observable<IctuFile[]> {
		const _queryParams : IctuQueryParams    = Object.assign( {
			limit : -1 ,
			paged : 1
		} , queryParams );
		const conditions : IctuConditionParam[] = [
			{ conditionName : 'id' , condition : IctuQueryCondition.equal , value : parent_id.toString( 10 ) } ,
			{ conditionName : 'parent_id' , condition : IctuQueryCondition.equal , value : parent_id.toString( 10 ) , orWhere : 'and' }
		];
		const params : HttpParams               = paramsConditionBuilder( conditions , new HttpParams( { fromObject : _queryParams } ) );
		return this.http.get<DtoObject<IctuFile[]>>( this.awsApi , { params } ).pipe(
			map( ( response : DtoObject<IctuFile[]> ) : IctuFile[] => response.data )
		);
	}
	
	public query( conditions : IctuConditionParam[] , queryParams? : IctuQueryParams , subpath : string = '' ) : Observable<DtoObject<IctuFile[]>> {
		const params : HttpParams = paramsConditionBuilder( conditions , new HttpParams( { fromObject : queryParams || {} } ) );
		const route : string      = subpath ? [ this.awsApi , subpath ].join( '' ) : this.awsApi;
		return this.http.get<DtoObject<IctuFile[]>>( Helper.removeLastSlashes( route ) , { params } );
	}
	
	public uploadToLibrary( file : File , folderID : number ) : Observable<UploadInfo> {
		return this.uploadWithProgress( file , { parent_id : folderID } , this.awsApi );
	}
	
	public deleteAwsItemByID( itemID : number ) : Observable<number> {
		const route : string = [ this.awsApi , itemID ].join( '' );
		return this.http.delete<DtoObject<number>>( route ).pipe(
			map( ( response : DtoObject<number> ) : number => response.data )
		);
	}
}
